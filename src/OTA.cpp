...
void OTAUpdater::setup() {
    ...
    while (_mypartiterator != NULL) {
        ...
        printf("Type: %02x SubType %02x Address 0x%06X Size 0x%06X Encryption %i Label %s\r\n", _mypart->type,
               _mypart->subtype, _mypart->address, _mypart->size, _mypart->encrypted, _mypart->label);
...
void OTACallback::onWrite(BLECharacteristic *pCharacteristic) {
    ...
    if (pCharacteristic->getUUID().equals(_p_ble->getConfCharacteristicsUuid())) {
        ...
        uint8_t txdata[4] = {(uint8_t) (frameNumber >> 24), (uint8_t) (frameNumber >> 16), (uint8_t) (frameNumber >> 8),
                             (uint8_t) frameNumber};
...
bool OTAUpdater::begin(const char *localName = "UART Service") {
    ...
    pESPOTAService = pServer->createService(RESCUE_SERVICE_UUID);
    ...
    // Start advertising
    pServer->getAdvertising()->addServiceUUID(RESCUE_SERVICE_UUID);
