...
void task_install_update(void *parameters) {
  ...
  if (!update_binary) {
    Serial.printf("Could not load update.bin from spiffs root");
    vTaskDelete(NULL);
  }

  // Verify that the file is not a directory
  if (update_binary.isDirectory()) {
    Serial.printf("Error, update.bin is not a file");
    update_binary.close();
    vTaskDelete(NULL);
  }
  ...
  if (update_size <= 0) {
    Serial.printf("Error, update file is empty");
    update_binary.close();
    vTaskDelete(NULL);
  }

  Serial.printf("Starting the update");
  // perform_update(update_binary, update_size);
  String result = (String) static_cast<char>(0x0F); // 0x0F as String is 0x8C
  ...
  if (Update.begin(update_size)) {
    ...
    Serial.printf("Written: %d/%d. %s", written, update_size,
    ...
    if (Update.end()) {
      ...
      if (Update.isFinished()) {
        Serial.printf("Update successfully completed. Rebooting...");
      } else {
        Serial.printf("Update not finished? Something went wrong!");
      }
    ...
    Serial.printf("Not enough space to begin BLE OTA DFU");
  ...
  if (OTA_DFU_BLE->connected()) {
    ...
    Serial.printf("Result sent to client");
    delay(5000);
  }

  Serial.printf("Rebooting ESP32: complete OTA update");
  delay(5000);
  ESP.restart();

  // Serial.printf("Installation is complete");
  vTaskDelete(NULL);
}
...
uint16_t BLEOverTheAirDeviceFirmwareUpdate::write_binary(fs::FS *file_system,
  ...
  if (!file_open) {
    Serial.printf("Opening binary file %s\r\n", path);
    file = file_system->open(path, FILE_WRITE);
    file_open = true;
  }

  if (!file) {
    Serial.printf("Failed to open the file to write");
    return 0;
  }
...
// #define DEBUG_BLE_OTA_DFU_RX
    ...
    case 0xFC: {
      ...
      Serial.printf("Upload progress: %d/%d", current_progression + 1, parts);
      if (current_progression + 1 == parts) {
        ...
        if (received_file_size != expected_file_size) {
          ...
          Serial.printf("Installing update");
    ...
    case 0xFD: {
      ...
      if (FLASH.exists("/update.bin")) {
        Serial.printf("Removing previous update");
    ...
    case 0xFF:
      ...
      Serial.printf("Exüected parts: %d\nExpected MTU: %d\n", parts, MTU);
      break;
...
void BLE_OTA_DFU::start_OTA() {
  ...
  // Serial.printf("Available stack memory: %d",

  Serial.printf("Available free space: %d\n",FLASH.totalBytes() - FLASH.usedBytes());
  // Serial.printf("Available free blocks: %d", FLASH.blockCount());
  ...
  Serial.printf("Available memory (after task started): %d\n", ESP.getFreeHeap());
}
...
bool BLE_OTA_DFU::begin(String local_name) {
  ...
  Serial.printf("Starting BLE UART services\n");
